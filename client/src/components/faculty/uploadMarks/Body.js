...
import { getStudent, uploadMark,uploadGraceMark } from "../../../redux/actions/facultyActions";
...
const Body = () => {
  ...
  const handleGrace = (value) => {
    setGraceMarks((prev) => value);
  };
  ...
  const uploadGraceMarks = (e) => {
    setError({});
    dispatch(
      uploadGraceMark(graceMarks, value.department, value.section, value.year, value.test)
    );
  };
            ...
            <div className="flex justify-between">
              {search && Object.keys(error).length === 0 && (
                <div className="">
                  <button
                    onClick={uploadMarks}
                    className={`${classes.adminFormSubmitButton} bg-blue-500 mt-5 ml-[5rem]`}
                  >
                    Upload
                  </button>
                </div>
              ...
              {search && Object.keys(error).length === 0 && (
                <div className="" style={{float:"right"}}>
                  {search && Object.keys(error).length === 0 && (
                    <input
                      onChange={(e) => handleGrace(e.target.value)}
                      placeholder="0"
                      className="col-span-2 border-2 w-24 px-2 h-8"
                      type="text"
                    />
                  ...
                  <button
                    onClick={uploadGraceMarks}
                    style={{ width: "150px" }}
                    className={`${classes.adminFormSubmitButton} bg-blue-500 mt-5 ml-[1rem]`}
                  >
                    Give Grace Marks
                  </button>
                </div>
